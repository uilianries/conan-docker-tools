...
if grep clang /home/conan/.conan/profiles/default ; then
    if grep compiler.version=10 /home/conan/.conan/profiles/default ; then
        conan create install imagl/${IMAGL_VERSION}@ --build -s compiler.libcxx=libc++ -s build_type=Release
        conan create imagl/${IMAGL_VERSION}@ --build -s compiler.libcxx=libc++ -s build_type=Release -o imagl:shared=True
    ...
    conan imagl/${IMAGL_VERSION}@ --build -s compiler.libcxx=libstdc++ -s build_type=Release
    conan imagl/${IMAGL_VERSION}@ --build -s compiler.libcxx=libstdc++11 -s build_type=Release -o imagl:shared=True
